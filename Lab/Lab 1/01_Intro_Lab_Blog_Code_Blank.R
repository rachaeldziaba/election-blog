...
d_popvote %>%
  ggplot(aes(x = year, y = pv2p, color = party)) + geom_line() +
  scale_color_manual(values = c("dodgerblue4", "firebrick1"))

#MAKE SURE COLOR IS IN AESTHETIC FOR DIFFERENT LINES TO BE DIFFERENT COLORS, 
#set colors to switch dem and rep colors#
...
pv2p_2024_states <- pv2p_2024_states %>%
  mutate(year = 2024,
         winer = ifelse(R_pv2p_2024 > D_pv2p_2024, "R", "D")) %>%
