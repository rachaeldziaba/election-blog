...
```{r, echo = T, results = 'hide'}
state <- read.csv("state_2pv_1948_2020.csv")
national <- read.csv("nat_pv_1860_2020.csv")
...
```{r}
national_mutate <- national %>%
  mutate(dem_tpv=100*npv_democrat/(npv_democrat+npv_republican), 
...
state_select %>%
  ...
  group_by(state) %>%
  summarize(state_average=mean(two_party_vote_share)) %>%
...
national_mutate_filter_long <- national_mutate_filter %>%
  pivot_longer(cols = c(dem_tpv, rep_tpv),
               names_to = "party",
               values_to = "national_two_party_vote_share") %>%
  mutate(party = ifelse(party=="dem_tpv", "Democrat", "Republican"))
...
```{r}
state_select_wide <-state_select %>%
  pivot_wider(names_from = c(party, state), values_from = c(two_party_vote_share))

head(state_select)
head(state_select_wide)

combined_data_wide <- full_join(state_select_wide, national_mutate_filter, by = "year")
...
```{r}
summary(lm(dem_tpv ~ Democrat_Florida, data = combined_data_wide))
```
...
multi_model <- lm(dem_tpv ~ Democrat_Florida + `Democrat_New York`, data = combined_data_wide)
...
```{r}
newdata <- data.frame(Democrat_Florida = 48, Democrat_New_York = 60) %>%
  rename("Democrat_New York" = Democrat_New_York)

predict(multi_model, newdata = newdata, type = "response")
```
...
```{r}
write.csv(combined_data_wide, "data/combined_popular_vote_wide.csv")
write.csv(combined_data, "data/combined_popular_vote_long.csv")
```
...
```{r}
ggplot(combined_data_wide, aes(x = Democrat_Florida)) +
...
```{r}
ggplot(combined_data_wide, aes(x= Democrat_Florida, y = dem_tpv, label = year)) +
  geom_label() +
  xlab("Two-Party Democratic Vote Share in Florida") +
  ylab("Two-Party National Democratic Vote Share") +
  ggtitle("Democratic Performance in Florida and National Vote Share")
```
